...
# ============== M1. 不確定性視覺化 modifier ==============
def uncertainty_modifier_examples():
    """RFC 2026-06-01 採納範例。
    A: 時序預測(漸層帶);B: 少量類別(error bar + 不對稱 CI)。
    詳細規範見 references/M1-uncertainty-modifier.md。"""

    # ----- A. 時序預測:類流感就診人次 + 4 週預測區間 -----
    ...
    ax.set_title("時序預測 ─ 漸層帶 + 預測虛線", loc="left")
    ax.legend(loc="upper left", fontsize=9, ncol=4)
    ax.set_ylim(bottom=0)
    save(fig, "m1a-uncertainty-trailing-band")

    # ----- B. 少量類別:各年齡組重症率 + 不對稱 95% CI -----
    ...
    # 規則 12: matplotlib capsize=4 建議值
    ...
    ax.set_title("少量類別 ─ error bar(主色更深版,不對稱 CI)",
                 loc="left")
    ax.set_ylim(bottom=0)
