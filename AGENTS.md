...
疫情資料視覺化指引——以 `#739A6D` Sage Green 為主色的完整色彩系統 + 10 種圖表規範 + AI agent skill 套件。
...
- **直條圖 Y 軸**必須從零開始,**不可截斷座標軸誤導比例**(折線/區域圖若變化才是訊息,zero baseline 可選但需明確標示)
- **移動平均**用 **trailing 7 日**(本日含前 6 日,即 i-6 到 i)── 對齊 WHO/CDC 通用慣例
