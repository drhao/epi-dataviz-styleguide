...
3. **跨年度同期比較**(時間維度):**5-6 年以上**每年一 panel,以週次對齊比較波形 ── 既有 02c 是 2 序列疊一張(< 4 年仍以此為主),本 RFC 處理「5-6 年以上」場景
...
- **只有 2-3 個 panel**:多數情境下直接放同一張圖比較即可,不必拆 panel。**但若一張圖同時呈現多指標 / 多資訊複雜度過高**(例:一張圖含 3 條折線 + 2 個區間帶 + annotation),拆成 2-3 個 panel 簡化是合理選擇
...
- ✓ **#R3 跨年度比較門檻**:**5-6 年以上**才用 M2;< 4 年仍以 02c 風格(疊一張 + 灰色歷史範圍帶)為主。Use case 3 與規則描述已更新
- ✓ **#R4 02c 處置**:**兩者並存**。02c 處理少量序列(2-3 年同期),M2 處理多 panel(5-6 年以上)。Active 後不替換 02c,僅在 M2 reference 內加 cross-link 指引「< 4 年用 02c 風格」
